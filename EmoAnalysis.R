...
text <- readLines("/Users/yuronan/develop/code/open-source/Understanding-Emotions/lars-reimann:api-editor-issue-comment.csv")
TextDoc <- Corpus(VectorSource(text))
size <- (length(text)) - 1
...
barplot(dtm_d[1:5, ]$freq,
  las = 2, names.arg = dtm_d[1:5, ]$word,
...
set.seed(1234)
wordcloud(
  words = dtm_d$word, freq = dtm_d$freq, min.freq = 5,
...
# disgust <- text[disgust_items]
# print(disgust)
#
#
# sadness_items <- which(d$sadness > 0)
# sadness <- text[sadness_items]
# print(sadness)
#
# anticipation_items <- which(d$anticipation > 0)
# anticipation <- text[anticipation_items]
# print(anticipation)
#
surprise_items <- which(d$surprise > 0)
surprise <- text[surprise_items]
print(surprise)
#
